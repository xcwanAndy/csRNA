...
/******************************* Init wqe *********************************/
struct ibv_wqe *IbvTestServer::init_rcv_wqe (struct ibv_context *ctx, int num) {
    ...
    for (int i = 0; i < num; ++i) {
        ...
        DPRINTF(IbvTestServer, "[test requester] init_rcv_wqe: len is %d\n", wqe[i].length);
...
struct ibv_wqe *IbvTestServer::init_snd_wqe (struct ibv_context *ctx, struct rdma_cr *cr_info, int num, uint16_t dlid) {
...
#define TRANS_SEND_DATA "hello RDMA Send!"
    ...
    for (int i = 0; i < num; ++i) {
        ...
        //DPRINTF(IbvTestServer, "[test requester] init_snd_wqe: string is %s, string vaddr is 0x%lx, start vaddr is 0x%lx\n",
                //string, (uint64_t)string, (uint64_t)(&string));

        wqe[i].length = sizeof(struct rdma_cr);
        DPRINTF(IbvTestServer, "[test requester] init_snd_wqe: len is %d\n", wqe[i].length);
        wqe[i].mr = ctx->cm_mr;
...
struct ibv_wqe *IbvTestServer::init_rdma_write_wqe (struct ibv_mr* lmr) {
...
struct ibv_wqe *IbvTestServer::init_rdma_read_wqe (struct ibv_mr* req_mr, struct ibv_mr* rsp_mr, uint32_t qkey) {
...
#define TRANS_RRDMA_DATA "hello RDMA Read!"
    ...
    DPRINTF(IbvTestServer, "[test requester] init_snd_wqe: len is %d, addr: 0x%lx, key: %x\n", wqe->length, (uint64_t)rsp_mr->addr, rsp_mr->lkey);
    wqe->mr = req_mr;
    wqe->offset = offset;
    ...
    // DPRINTF(IbvTestServer, "[test requester] init_snd_wqe: rKey: 0x%x, rAddr: 0x%lx\n", wqe->rdma.rkey, wqe->rdma.raddr);
    return wqe;
}
/******************************* Init wqe *********************************/
...
/******************************* Poll Cpl *********************************/
struct cpl_desc ** IbvTestServer::malloc_desc() {
...
void IbvTestServer::poll_cpl() {
    //DPRINTF(IbvTestServer, "[DEBUG] poll_cpl: Waiting for cpl ...\n");
    void (IbvTestServer::*deal_cpl)(struct cpl_desc *) = NULL;
    for (struct ibv_cq *cq : cplWaitingList) {
        //DPRINTF(IbvTestClient, "Polling from cq: %d\n", cq->cq_num);
        if (cq->cq_num == res->ctx->cm_cq->cq_num) {
            deal_cpl = &IbvTestServer::rdma_listen_post;
        } else if (cq->cq_num == res->cq[0]->cq_num) {
            deal_cpl = &IbvTestServer::rdma_write_post;
        }
...
void IbvTestServer::rdma_write_pre() {
    ...
    for (int i = 0; i < res->num_qp; ++i) {
        ...
        DPRINTF(IbvTestServer, "IBV post write wqe to qp %d!\n", res->qp[i]->qp_num);
...
void IbvTestServer::rdma_write_post(struct cpl_desc *desc) {
    struct ibv_context *ctx = res->ctx;
    struct rdma_cr *cr_info;

    DPRINTF(IbvTestServer, "rdma_write_post: cpl_desc trans_type %d cq_num %d\n", desc->trans_type, desc->cq_num);

    if (desc->trans_type == IBV_TYPE_RDMA_WRITE) {
        DPRINTF(IbvTestServer, "rdma_write_post: ibv_poll_cpl recv %d bytes CR.\n", desc->byte_cnt);
        /* Fetch rdma_cr from cm_mr */
        cr_info = (struct rdma_cr *)malloc(sizeof(struct rdma_cr));
        struct rdma_cr *cr_tmp = ((struct rdma_cr *)(ctx->cm_mr->addr + ctx->cm_rcv_acked_off));
...
void IbvTestServer::config_rc_qp() {
    ...
    for (int i = 0; i < res->num_qp; ++i) {
        ...
        DPRINTF(IbvTestServer, "[test requester] ibv_modify_qp end!\n");
    }
}


void IbvTestServer::config_ud_qp (struct ibv_qp* qp, struct ibv_cq *cq, struct ibv_context *ctx, uint32_t qkey) {
    qp->ctx = ctx;
    qp->flag = 0;
    qp->type = QP_TYPE_UD;
    qp->cq = cq;
...
int IbvTestServer::main () {
    ...
    DPRINTF(IbvTestServer, "main function executing ...\n");
